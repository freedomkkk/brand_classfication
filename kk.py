...
def image_dir(imagename,classLabelVector):
    image_data = np.empty((len(imagename),1,128,192),dtype = 'float32')#建立空的四维张量类型32位浮点
    label = np.empty((len(imagename),),dtype = 'uint8')
    for i in range(len(imagename)):
       ...
       # print(new_path)
        ...
        res=cv2.resize(canny1,(192,128),interpolation=cv2.INTER_AREA)
        array = np.asarray(res,dtype='float32')
        array -= np.min(array)
        array /= np.max(array)
        image_data[i,:,:,:] = array
        label[i] = classLabelVector[i]-1
...
#        print child
    ...
    return data,label
#构建卷积神经网络
def cnn_model(train_data,train_label,test_data,test_label,test_imagename):
    model = Sequential()
#卷积层 12 × 120 × 120 大小
...
#池化层 24×29×29
    model.add(MaxPooling2D(
        ...
        strides = (2,2),
        border_mode = 'valid'))
    model.add(Convolution2D(48,3,3,border_mode = 'valid',dim_ordering = 'th'))
    model.add(Activation('relu'))
    ...
    result_label = model.predict_classes(test_data)
    outputFile = open('result1.csv', 'w', newline='')
    outputWriter = csv.writer(outputFile,delimiter=' ')
    for i in range(len(test_data)):
        outputWriter.writerow([test_imagename[i],str(result_label[i]+1)])
    outputFile.close()
...
if __name__=='__main__':
    ...
    test_label = cnn_model(train_data,train_label,test_data,test_label,test_imagename)
    print(test_label)
